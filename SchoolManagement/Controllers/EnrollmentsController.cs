...
namespace SchoolManagement.Controllers
{
    public class EnrollmentsController : Controller
    {
        ...
        public async Task<JsonResult> AddStudent([Bind(Include = "CourseID,StudentID")] Enrollment enrollment) 
        {
            try
            {
                var isEnrolld = db.Enrollments.Any(q => q.CourseID == enrollment.CourseID && q.StudentID == enrollment.StudentID); 
                if (ModelState.IsValid && isEnrolld ==false)
                {
                    db.Enrollments.Add(enrollment);
                    await db.SaveChangesAsync();
                    return Json(new { IsSuccess = true, Message = "Student Added Successfully" }, JsonRequestBehavior.AllowGet);
                }
                return Json(new { IsSuccess = false, Message = "Student Already Added" }, JsonRequestBehavior.AllowGet);

            }
            catch (Exception)
            { 
                return Json(new { IsSuccess = false, Message = " SYSTEM FAILURE : Contact Admin" }, JsonRequestBehavior.AllowGet);
            }
        }
